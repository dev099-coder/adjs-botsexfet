				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pedidos emitidos para remover como admin! :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Pedidos emitidos para remover @${mentioned[0].split('@')[0]}\n Como Admin! _*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos emitidos para adicionar como admin! :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedidos emitidos para adicionar como admin! : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'citações':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui está o número do meu dono, mana, salve mano, salve depois',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'testime':
					...
					break
				case 'semoji':
					if (args.length < 1) return reply('Amojia se importa?')
                                        if (!isUser) return reply(mess.only.daftarB)
					ranp = getRandom('.png')
					rano = getRandom('.webp')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencione tudo 〙✪══\n╠➥'+teks+'╚═〘 Lightbot 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 Aviso do LIGHTBOT: 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 AVISO DO LIGHTBOT: 」*\n\n${body.slice(4)}`)
						}
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país, mana')
					try {
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'levou ban😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				if (args.length < 1) return reply('Qual é o bug, irmão?')
					tek = body.slice(10)
					bug = {
					text: `*[BUG REPORT]*\n\n*Remetente q :* @${sender.split("@")[0]}\n*A que horas :* ${time}\n*mensagem :* ${tek}`,
					contextInfo: { mentionedJid: [sender] }
					}
					client.sendMessage(nomorOwner, bug, text, {quoted: mek})
					client.sendMessage(from, 'Seu relatório foi enviado para o proprietário do BOT, relatórios palsu/main2 tidak akan ditanggapi.', text, {quoted: mek})
					break
               case 'oque':
               client.updatePresence(from, Presence.composing) 
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					setTimeout( () => {
					client.sendMessage(from, '_20 segundos restantes_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					...
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}cadastro nome|idade\nExemplo : ${prefix}cadastro Lightbot|1`)
				...
				if (isUser) return reply(mess.only.daftarB)
					watak = body.slice(1)
					const wa =['peny ayang','horas pem','instruções','Pem aaf','Pen urut','Ba ik','bap eran','Baik Hati','peny abar','Uw u','top deh, poko knya','Suka Memb antu']
